...
data.reconstructionSettings['processState'] = 'struct+angio+ps+hsv'#'+kspace+stokes'
if b_change_editsetting:
    data.reconstructionSettings['spectralBinning'] = False
    data.reconstructionSettings['depthIndex'] = [0, 0] # [0,0] #
...
if b_change_editsetting:
    ...
    data.hsvSettings['structWeight'] = np.array([100, 180])
    data.hsvSettings['dopWeight'] = np.array([190, 250])
...
viewer.add_image(np.squeeze(cp.asnumpy(SVN1)), name='SVN1', rgb=False, contrast_limits=(-1,1), colormap='viridis' )
viewer.add_image(np.squeeze(cp.asnumpy(SVN2)), name='SVN2', rgb=False, contrast_limits=(-1,1), colormap='viridis' )
viewer.add_image(np.squeeze(data.processedData['oa'][:,:,:,None]), name='oa', rgb=False, colormap='plasma' )
...
viewer4.add_image(log_intensity_disp, name='log_intensity_disp', rgb=False, colormap='gray', contrast_limits=(0,60))
